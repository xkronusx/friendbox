...
_mergerfs_write_fstab() {
  ...
  echo "${branch_list}  ${pool_path}  fuse.mergerfs  defaults,allow_other,use_ino,uid=${uid},gid=${gid},sync_read,auto_cache,func.getattr=newest,dropcacheonclose=true,category.create=ff,minfreespace=0,moveonenospc=true,fsname=mergerpool  0  0" >> /etc/fstab
...
_mergerfs_remount() {
  ...
  local mount_out
  mount_out=$(mergerfs \
    -o allow_other,use_ino,uid=${uid},gid=${gid},sync_read,auto_cache,func.getattr=newest,dropcacheonclose=true,category.create=ff,minfreespace=0,moveonenospc=true,fsname=mergerpool \
...
_traefik_write_config() {
  ...
  local tls_block=""
  # For DuckDNS: the entrypoint-level certResolver+domains approach is broken in
  # Traefik v3 (confirmed bug: github.com/traefik/traefik/issues/12109).
